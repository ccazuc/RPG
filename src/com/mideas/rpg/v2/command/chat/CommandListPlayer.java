...
public class CommandListPlayer extends Command {
	...
	public void read() {
		...
		int i = 0;
		while(i < number) {
			ChatFrame.addMessage(new Message(ConnectionManager.getConnection().readString(), false, MessageType.SELF, Joueur.convClassTypeToColor(ClassType.values()[ConnectionManager.getConnection().readChar()])));
			i++;
		}
	}
