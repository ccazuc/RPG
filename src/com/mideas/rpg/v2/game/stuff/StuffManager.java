...
public class StuffManager {
	...
	public static void loadStuffs() throws SQLException, CloneNotSupportedException {
		JDOStatement statement = Mideas.getJDO().prepare("SELECT id, name, class, sprite_id, type, armor, stamina, mana, critical, strength, sellprice FROM stuff");
		statement.execute();
		while(statement.fetch()) {
			...
			Stuff newPiece = new Stuff(type, id, name, critical, strength, stamina, armor, mana, "", "", "", "", 0, sellPrice, 0, "");
	...
	private static ClassType[] getClasses(short type) {
		int i = 0;
		int count = 0;
		while(i < 10) {
			if((type & (1 << i)) != 0) {
