...
public class AddFriendInputFrame {
	...
	private final static int TITLE_WIDTH = TTF2.addingFriendInput.getWidth("Indiquez le nom de l'ami à ajouter :");
	...
	public static void draw() {
		background.draw();
		TTF2.addingFriendTitle.drawStringShadow(Display.getWidth()/2-TITLE_WIDTH/2, Display.getHeight()/2-350*Mideas.getDisplayYFactor(), "Indiquez le nom de l'ami à ajouter :", Color.white, Color.black, 1, 0, 0);
		inputBar.draw();
		...
		if(System.currentTimeMillis()%1000 < 500) {
			Draw.drawColorQuad(Display.getWidth()/2-76*Mideas.getDisplayXFactor()+input.getCursorShift(), Display.getHeight()/2-317*Mideas.getDisplayYFactor(), 5*Mideas.getDisplayXFactor(), 15*Mideas.getDisplayYFactor(), Color.white);
		}
		acceptButton.draw();
		cancelButton.draw();
	}
