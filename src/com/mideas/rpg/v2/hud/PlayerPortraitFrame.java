...
public class PlayerPortraitFrame {
	...
	public static void draw(Unit joueur, float x, float y) {
		...
		FontManager.get("FRIZQT", 12).drawStringShadow((int)(x+135*Mideas.getDisplayXFactor()-FontManager.get("FRIZQT", 12).getWidth(joueur.getName())/2), (int)(15*Mideas.getDisplayYFactor()+y), joueur.getName(), Colors.ORANGE, Colors.BLACK, 1, 1, 1);    
		FontManager.get("FRIZQT", 11).drawBegin();
		drawHealthText(joueur, (int)x, (int)y);
		drawManaText(joueur, (int)x, (int)y);
		FontManager.get("FRIZQT", 11).drawStringShadow(x+16-FontManager.get("FRIZQT", 11).getWidth(String.valueOf(joueur.getLevel()))/2, y+55, String.valueOf(joueur.getLevel()), YELLOW, Colors.BLACK, 1, 0, 0);
	...
	private static void drawHealthText(Unit joueur, int x, int y) {
		for(int i=129;i<132;i++) {
			for(int j=32;j<35;j++) {
				FontManager.get("FRIZQT", 11).drawStringPart(x+(int)(i*Mideas.getDisplayXFactor()-FontManager.get("FRIZQT", 11).getWidth(joueur.getHealthText())/2), y+j*Mideas.getDisplayYFactor(), joueur.getHealthText(), Colors.BLACK);
			}
		}
		FontManager.get("FRIZQT", 11).drawStringPart(x+(int)(130*Mideas.getDisplayXFactor())-FontManager.get("FRIZQT", 11).getWidth(joueur.getHealthText())/2, y+33*Mideas.getDisplayYFactor(), joueur.getHealthText(), Colors.WHITE);
	...
	private static void drawManaText(Unit joueur, int x, int y) {
		for(int i=129;i<132;i++) {
			for(int j=43;j<46;j++) {
				FontManager.get("FRIZQT", 11).drawStringPart(x+(int)(i*Mideas.getDisplayXFactor()-FontManager.get("FRIZQT", 11).getWidth(joueur.getManaText())/2), y+j*Mideas.getDisplayYFactor(), joueur.getManaText(), Colors.BLACK);
			}
		}
		FontManager.get("FRIZQT", 11).drawStringPart(x+(int)(130*Mideas.getDisplayXFactor())-FontManager.get("FRIZQT", 11).getWidth(joueur.getManaText())/2, y+44*Mideas.getDisplayYFactor(), joueur.getManaText(), Colors.WHITE);
