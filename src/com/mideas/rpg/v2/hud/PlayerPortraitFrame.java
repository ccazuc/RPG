...
public class PlayerPortraitFrame {
	...
	private static void drawHealthText(Joueur joueur, int x, int y) {
		...
		for(int i=129;i<132;i++) {
			for(int j=32;j<35;j++) {
				TTF2.hpAndMana.drawString(x+(int)(i*Mideas.getDisplayXFactor()-TTF2.hpAndMana.getWidth(stamina)/2), y+j*Mideas.getDisplayYFactor(), stamina, Color.black);
			}
		}
		TTF2.hpAndMana.drawString(x+(int)(130*Mideas.getDisplayXFactor())-TTF2.hpAndMana.getWidth(stamina)/2, y+33*Mideas.getDisplayYFactor(), stamina, Color.white);
